...
class MW_RewardPoints_Model_Customer extends Mage_Core_Model_Abstract
{
	...
	public function fetchCash($type,$message,$amount){
				...
				if ($customerId){
					...
					if ($rewCust && $rewCust->getCustomerId()){
						...
						if ($this->getMwRewardPoint()>=$amount){
							...
							$this->save();
							Mage::getModel("rewardpoints/cashout")->cashOutRequest($type,$message,$amount);
							Mage::getSingleton("core/session")->addSuccess("Congrats your $amount kinzacash have been cashed out");
